...
def unfold_res_dict(res_dict):
    ...
    parts_vecs_z = res_dict['parts_enc']
    ...
    if 's_weights_pairs' in res_dict.keys():
        ...
        return rec_imgs, rec_protos, dists, s_weights, parts_vecs_z, feature_vecs_z, proto_vecs, agg_protos, pair_s_weights, s_weights_pair
    elif 'dists_pairs' in res_dict.keys():
        dists_pair = res_dict['dists_pairs']
        return rec_imgs, rec_protos, dists, s_weights, parts_vecs_z, feature_vecs_z, proto_vecs, agg_protos, pair_s_weights, dists_pair
    else:
        return rec_imgs, rec_protos, dists, s_weights, parts_vecs_z, feature_vecs_z, proto_vecs, agg_protos, pair_s_weights


# TODO: must be updated to part prototypes!!!
def plot_prototypes(model, prototype_vectors, writer, config, step=0):
    ...
    for group_id in range(config['n_prototype_groups']):
        prototype_imgs = model.dec_wrapper(
            prototype_vectors[group_id], Proto=True).detach().cpu()
