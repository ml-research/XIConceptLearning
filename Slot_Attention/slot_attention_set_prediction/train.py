...
def run(net, loader, optimizer, criterion, writer, args, train=False, epoch=0, pool=None):
    ...
    for i, sample in tqdm(enumerate(loader, start=epoch * iters_per_epoch)):
        ...
        if train:
            # # manual exponential lr scheduler
            # for param_group in optimizer.param_groups:
            #     param_group['lr'] = args.lr * 0.5**(i/50000) # learning_rate * decay_rateˆ(step/ decay_steps))
            ...
            if i % iters_per_epoch == 0:
                ...
                ap = [
                    utils.average_precision_clevr(preds_all.detach().cpu().numpy(),
    ...
    # # store ap per ap_log epochs
    # if not train and (((epoch + 1) % args.ap_log == 0) or epoch == 0):
    #     ap = [
    #         utils.average_precision_clevr(preds_all.detach().cpu().numpy(),
    #                                       target_all.detach().cpu().numpy(), d)
    #         for d in [-1]  # since we are not using 3D coords #[-1., 1., 0.5, 0.25, 0.125]
...
def main():
    ...
    # Create RTPT object
    rtpt = RTPT(name_initials='WS', experiment_name=f"Clevr Hans Slot Att",
