...
class MLP(nn.Module):
    def __init__(self, hidden_channels):
        ...
        self.network = nn.Sequential(
            # TODO groups should be #hidden (cf paper ) but weights of pre trained tf models suggest that it is 1
            # nn.Conv1d(hidden_channels, hidden_channels, 1, stride=1, groups=1),
            #nn.Conv1d(hidden_channels, hidden_channels, 1, stride=1, groups=hidden_channels),
...
class SlotAttention_model(nn.Module):
    def __init__(self, n_slots, n_iters, n_attr,
        ...
        # self.layer_norm = nn.InstanceNorm1d(encoder_hidden_channels, eps=1e-05, affine=True)#nn.LayerNorm(encoder_hidden_channels, eps=1e-05)
        self.layer_norm = nn.LayerNorm(encoder_hidden_channels, eps=1e-05)#nn.LayerNorm(encoder_hidden_channels, eps=1e-05)
