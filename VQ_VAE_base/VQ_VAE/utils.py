...
def plot_examples(log_samples, model, writer, config, step=0, rec_protos=None, cls=False):
    model.eval()
    ...
    if rec_protos is None:
        if not cls:
            # for VQ VAE
            vq_loss, recons, perplexity = model.forward(imgs)
        else:
            # for GVQ VAE Cls
            vq_loss, recons, perplexity, embeddings, embedding_ids, preds = model.forward(imgs)
            preds = preds.detach().cpu().permute(1, 0, 2)
            max_idx = torch.argmax(preds, dim=2, keepdim=True)
            one_hots = torch.FloatTensor(preds.shape)
            one_hots.zero_()
            one_hots.scatter_(2, max_idx, 1)
            preds = torch.flatten(one_hots, start_dim=1)
            embedding_ids = embedding_ids.detach().cpu()
    ...
    if not cls:
        ...
        a[0][0].text(0, -2, s='input', fontsize=10)
        a[1][0].text(0, -2, s='recon z', fontsize=10)
        a[2][0].text(0, -2, s='emb ids 1', fontsize=10)
        a[3][0].text(0, -2, s='emb ids 2', fontsize=10)
        a[4][0].text(0,-2, s='prediction', fontsize=10)
    ...
    for i in range(examples_to_show):
        ...
        if cls:
            # plot embedding ids for first group
            img = a[2][i].imshow(embedding_ids[i, 0].squeeze().numpy(),
                           vmin=0., vmax=4.)
            a[2][i].axis('off')

            # plot embedding ids for second group
            img = a[3][i].imshow(embedding_ids[i, 1].squeeze().numpy(),
                           vmin=0., vmax=4.)
            a[3][i].axis('off')

            img = a[4][i].imshow(np.array([labels[i].detach().cpu().numpy(), preds[i].numpy()]),
                           cmap='gray',
                           vmin=0., vmax=1.)
            a[4][i].axis('off')
