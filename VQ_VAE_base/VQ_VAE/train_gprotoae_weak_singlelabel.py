...
from VQ_VAE.models.gproto_ae_weak_singlelabel import GProtoAEPair
...
def train(model, data_loader, log_samples, optimizer, scheduler, writer, config):
    ...
    for e in range(0, config['epochs']):
        ...
        start = time.time()
        loss_dict = dict(
            {'recon_loss_z': 0, 'recon_loss_proto': 0, 'loss': 0, 'vq_loss': 0, 'pair_loss': 0})
        ...
        for i, batch in enumerate(data_loader):
            ...
            vq_loss, pair_loss, imgs_recon_z, imgs_recon_proto, _, _, _, distances = model(imgs, labels)

            # plot training images
            if i == 0 and ((e + 1) % config['save_step'] == 0 or e == config['epochs'] - 1 or e == 0):
                utils.plot_train_examples(imgs, imgs_recon_proto, distances, writer, config, step=e)
            ...
            # reconstruciton loss for protos
            recon_loss_proto0 = F.mse_loss(imgs_recon_proto[0], imgs0)
            recon_loss_proto1 = F.mse_loss(imgs_recon_proto[1], imgs1)
            ave_recon_loss_proto = (recon_loss_proto0 + recon_loss_proto1) / 2

            loss = config['lambda_recon_z'] * ave_recon_loss_z + \
                   config['lambda_recon_proto'] * ave_recon_loss_proto + \
                   vq_loss + \
            ...
            loss_dict['pair_loss'] += pair_loss.item() if config['lambda_pair'] > 0. else 0.
...
def main(config):
    ...
    # model setup
    _model = GProtoAEPair(num_hiddens=32, num_residual_layers=2, num_residual_hiddens=32,
