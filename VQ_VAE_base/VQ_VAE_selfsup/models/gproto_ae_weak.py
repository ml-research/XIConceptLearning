...
class VectorQuantizerPair(nn.Module):
	...
	def forward_pair(self, inputs0, inputs1, shared_masks=None, std=0.):
		# shared_masks: [G, B]
		# [B, G, L] --> [G, B, L]
		...
		for group_id in range(self.num_groups):
			...
			if std:
				...
				distances1 += torch.normal(torch.zeros_like(distances1), std)

			distances0_all.append(distances0)
			distances1_all.append(distances1)

			# get prototype indices for each image
			encoding_indices0 = torch.argmin(distances0, dim=1).unsqueeze(dim=1)
			encoding_indices1 = torch.argmin(distances1, dim=1).unsqueeze(dim=1)

			# for those groups for which the attribute should be shared between the two images, set the prototype
			# indices of the second image to those of the first image
			encoding_indices1[bool_share.unsqueeze(dim=1)] = encoding_indices0[bool_share.unsqueeze(dim=1)]
...
class GProtoAEPair(nn.Module):
	...
	def forward(self, imgs, k=None, std=0.):
		if type(imgs) is not tuple:
			return self.forward_inference_single(imgs)
		else:
			return self.forward_pairs(imgs, k=k, std=std)

	def forward_pairs(self, imgs, k, std):
		(x0, x1) = imgs
		...
		# perform vector quantization with prototypes
		vq_loss, pair_loss, quantized, perplexity, embeddings, \
		embedding_ids_one_hot, distances = self._vq_vae.forward_pair(z0, z1, shared_masks, std=std)
	...
	# TODO: remove std here and only have it on cosine similarity of VQ part
	def _comp_shared_group_ids(self, inputs0, inputs1, k=None):
		...
		# if std:
		# 	distances += torch.normal(torch.zeros_like(distances), std)  # [B, G]
