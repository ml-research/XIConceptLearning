...
def plot_train_examples(imgs, recon_imgs, distances, emb_distances, writer, config, step=0):
    n_examples_to_show = 4

    # distances_aug = distances[0].permute(1, 0)
    # distances_neg = distances[1].permute(1, 0)
    distances_neg = distances.permute(1, 0)
    ...
    for triplet_idx in range(n_examples_to_show):
        ...
        # a[7].set_title('dist augm', fontsize=10)
        a[8].set_title('dist neg', fontsize=10)
        ...
        # img = a[7].imshow(distances_aug[triplet_idx].unsqueeze(dim=1).detach().cpu().numpy(),
        #                cmap='gray')
        # a[7].axis('off')

        img = a[8].imshow(distances_neg[triplet_idx].unsqueeze(dim=1).detach().cpu().numpy(),
                       cmap='gray')
        a[8].axis('off')

        img = a[9].imshow(emb_distances[0][triplet_idx].detach().cpu().numpy(),
                                   cmap='gray')
        # vmin=-1., vmax=1.)
        a[9].axis('off')
        img = a[11].imshow(emb_distances[1][triplet_idx].detach().cpu().numpy(),
                                   cmap='gray')
        # vmin=-1., vmax=1.)
        a[11].axis('off')
...
def plot_train_examples_2(model, imgs, writer, config, step=0):
    n_examples_to_show = 4
    ...
    for triplet_idx in range(n_examples_to_show):
        ...
        if writer:
            img_save_path = os.path.join(config['img_dir'], f'{step:05d}' + f'_train_decoding_result_pair_{triplet_idx}_sanity.png')
            plt.savefig(img_save_path,
                        transparent=True,
                        bbox_inches='tight',
                        pad_inches=0)
            ...
            image = Image.open(img_save_path)
            image = TF.to_tensor(image)
            writer.add_image(f"train_rec/train_decoding_result_pair_{triplet_idx}_sanity", image, global_step=step)
