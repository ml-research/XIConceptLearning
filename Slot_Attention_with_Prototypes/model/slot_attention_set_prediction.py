...
def spatial_broadcast(slots, resolution):
    ...
    inter_shp = slots.shape
    grid = slots.reshape(-1, 1).repeat(1, resolution[0], resolution[1], 1).reshape(inter_shp[0], resolution[0],
                                                                             resolution[1], inter_shp[3])
...
# STEP 1: CNN to downsample image into features
...
class SoftPositionEmbed(nn.Module):
    ...
    def __init__(self, hidden_size, resolution):
        ...
        self.grid = nn.Parameter(torch.FloatTensor(build_grid(resolution)))
...
class SlotAttention_model(nn.Module):
    def __init__(self, n_slots, n_iters, n_attr,
        ...
        self.encoder_pos = SoftPositionEmbed(encoder_hidden_channels, (32, 32))
    ...
    def forward(self, x):
        ...
        if self.classifier:
            ...
            # print('out slot classifier', x.shape)
        return x
...
if __name__ == "__main__":
    ...
    net = SlotAttention_model(n_slots=10, n_iters=3, n_attr=18,
                              encoder_hidden_channels=64, attention_hidden_channels=128).to('cuda')
    output = net(x)
    print(output.shape)
