...
class SwapCatVae(CatVae):
    ...
    def compute_training_loss(self, batch, batch_idx):
        ...
        # intercept and mutate z, possibly swap
    ...
    def intercept_z(self, all_params, share_mask=None):
        ...
        new_args = self.swap_latents(z0_params, z1_params, share_mask)
    ...
    def compute_averaged(cls, z0_params, z1_params, share_mask, compute_average_fn: callable):
        ...
        # select averages
        ave_z0_logits = where_by_latent_variable_id(share_mask, ave_logits, z0_logits)
        ave_z1_logits = where_by_latent_variable_id(share_mask, ave_logits, z1_logits)
    ...
    @classmethod
    def swap_latents(cls, z0_params, z1_params, share_mask):
        def where_by_latent_variable_id(share_mask, z0_logits, z1_logits):
            # TODO: double check if gradient is passed through clone
            swap_logits_0 = torch.clone(z1_logits)
            swap_logits_1 = torch.clone(z0_logits)
            # for those variable coordinates which are not shared, use thes logits of the original data sample,
            # otherwise use the swapped logits
            swap_logits_0[~share_mask] = z0_logits[~share_mask]
            swap_logits_1[~share_mask] = z1_logits[~share_mask]
            return swap_logits_0, swap_logits_1

        M, K, z0_logits, z0_probs = z0_params
        M, K, z1_logits, z1_probs = z1_params

        # swap logits
        swap_z0_logits, swap_z1_logits = where_by_latent_variable_id(share_mask, z0_logits, z1_logits)

        # update event probabilities based on average logits
        z0_probs = F.softmax(swap_z0_logits, dim=-1)
        z1_probs = F.softmax(swap_z1_logits, dim=-1)

        # return values
        return z0_params.__class__(M, K, swap_z0_logits, z0_probs), z1_params.__class__(M, K, swap_z1_logits, z1_probs)
