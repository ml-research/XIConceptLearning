...
from disent.frameworks.vae.weaklysupervised import AdaCatVae

import BaseVAEs.utils_disent as utils
import BaseVAEs.data as data
# from Proto_Cat_VAE.models.ada_cat_vae import AdaCategoricalVAE # AdaProtoCategoricalVAE
...
def train(model, data_loader, log_samples, optimizer, scheduler, writer, config):
    ...
    for e in range(0, config['epochs']):
        ...
        print(model._distributions.temp)
...
def main(config):
    ...
    # model setup
    z_size = config['n_protos'] * config['n_groups']
    _model = AdaCatVae(make_optimizer_fn=lambda params: Adam(params, lr=1e-3),
                       ...
                       cfg=AdaCatVae.cfg(beta=4, n_categories=config['n_protos'],
                                         ...
                                         temp=config['temperature'], eps=1e-12, z_size=z_size,
