...
from pair_prototype_nongroup_to_group.model_min import Pair_RAE
...
def train(model, data_loader, log_samples, optimizer, scheduler, writer, config):
    ...
    for e in range(0, config['epochs']):
        ...
        for i, batch in enumerate(data_loader):
            ...
            rec_imgs, rec_protos, dists, min_weights, feature_vecs_z, \
            proto_vecs, agg_protos, pair_s_weights, s_weights_pairs = utils.unfold_res_dict(res_dict)
            ...
            if config['lambda_pair'] != 0:
                ...
                # minimize the difference between the prototypes chosen for the first group
                pair_loss = mse(s_weights_pairs[0][0], s_weights_pairs[1][0])
                # maximize the difference between the prototypes chosen for the second group
                pair_loss += 1./torch.exp(mse(s_weights_pairs[0][1], s_weights_pairs[1][1]))
                pair_loss /= 2.
