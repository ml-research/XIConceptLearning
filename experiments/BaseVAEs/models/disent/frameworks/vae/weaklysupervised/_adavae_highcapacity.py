...
class AdaVae_HighCapacity(BetaVae):
    ...
    def compute_averaged(cls, z0_params, z1_params, share_mask, compute_average_fn: callable):
        ...
        # select averages
        ave_z0_mean = torch.where(update_share_mask, ave_mean, z0_params.mean)
        ave_z1_mean = torch.where(update_share_mask, ave_mean, z1_params.mean)
        ave_z0_logvar = torch.where(update_share_mask, ave_logvar, z0_params.logvar)
        ave_z1_logvar = torch.where(update_share_mask, ave_logvar, z1_params.logvar)
        # return values
