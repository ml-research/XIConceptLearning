...
class iCSN(nn.Module):
    def __init__(
        ...
        self._discriminator = nn.Sequential(
            nn.Linear(self.n_groups * self.proto_dim + self.n_attrs, 512),
            nn.Mish(),
            nn.Linear(512, 512),
            nn.Mish(),
            nn.Linear(512, 1),
    ...
    def freeze_encoder(self, unfreeze=False):
        self._encoder.requires_grad = unfreeze
        print(f"Froze encoder")

    # ------------------------------------------------------------------------------------------------ #
    # fcts for forward passing

    def forward(self, imgs, shared_masks, group_id=0):
        ...
        if self.multiheads:
            z0_extra = (
                torch.stack(
                    [self.extra_mlps[i].forward(z0) for i in range(self.n_groups)]
                )
                .permute(1, 0, 2)
            ...
            z1_extra = (
                torch.stack(
                    [self.extra_mlps[i].forward(z1) for i in range(self.n_groups)]
                )
                .permute(1, 0, 2)
        ...
        z0_proto_recon = self.proto_decode_multiple(pred0, group_id)
        z1_proto_recon = self.proto_decode_multiple(pred1, group_id)
        z0_proto_recon_swap = self.proto_decode_multiple(pred0_swap, group_id)
        z1_proto_recon_swap = self.proto_decode_multiple(pred1_swap, group_id)
    ...
    def forward_single(self, imgs, discriminator=False, group_id=0):
        ...
        # x: [B, 3, 64, 64] --> [B, F, W, H] --> [B, D]
        z_enc, z_lin = self.encode(imgs)

        if self.multiheads:
            z_extra = (
                torch.stack(
                    [self.extra_mlps[i].forward(z_lin) for i in range(self.n_groups)]
                )
                .permute(1, 0, 2)
                .reshape((z_lin.shape[0], -1))
            )
        else:
            z_extra = self.extra_mlp(z_lin)
        ...
        if discriminator:
            ...
            # idx = torch.randperm(preds.nelement())
            # shuffled_preds = preds.view(-1)[idx].view(preds.size())
            # shuffled_preds = torch.roll(preds,1, dims=0)
            real_data = torch.cat((preds, z_flat), dim=1)
            # fake_data = torch.cat((shuffled_preds, z_flat), dim=1)
            fake_data = torch.cat(
                [
                    torch.cat((torch.roll(preds, i, dims=0), z_flat), dim=1)
        ...
        if discriminator:
            return preds, z_proto_recon, disc_real_pred, disc_fake_pred
        else:
    ...
    def forward_autoencoder(self, imgs):
        self.batch_size = imgs.shape[0]

        # x: [B, 3, 64, 64] --> [B, F, W, H] --> [B, D]
        z_enc, z_lin = self.encode(imgs)
        z_recon = self.decode(z_lin)
    ...
    def encode(self, x):
        z_enc = self._encoder(x)  # [B, F, W, H]
        z_lin = self._encoder_linear(z_enc.reshape(z_enc.shape[0], self.latent_flat))
        return z_enc, z_lin

    def decode(self, z):
        # reconstruct z
        z = self._decoder_linears[-1](z)

        recon = self._decoder(
            z.view(-1, self.latent_shape[0], self.latent_shape[1], self.latent_shape[2])
        )  # [B, 3, W, H]
        return recon

    def proto_decode(self, preds):
        # reconstruct z
        z_proto = self._decoder_linears[-1](preds)
        ...
        for group_id in range(self.n_groups):
            if group_id <= self.train_group:
                ...
                distances = torch.full(
                    (inputs0.shape[1], self.n_proto_vecs[group_id]),
                    1 / self.n_proto_vecs[group_id],
                    device=distances0_emb.device,
                )
                distances0_emb = torch.cat((distances0_emb, distances), dim=1)
                distances1_emb = torch.cat((distances1_emb, distances), dim=1)
                distances0_emb_swap = torch.cat((distances0_emb_swap, distances), dim=1)
                distances1_emb_swap = torch.cat((distances1_emb_swap, distances), dim=1)
