...
class VT(nn.Module):
    def __init__(
        ...
        self._decoder_proto = modules.Decoder(
        ...
        self._decoder_linear = nn.Sequential(
            nn.Linear(self.n_attrs + self.extra_dims, self.latent_flat),
    ...
    def forward(self, imgs, shared_masks):
        ...
        z0_lin, z0_vt = self.encode(x0)
        z1_lin, z1_vt = self.encode(x1)

        # this extracts the variation information, i.e. color jitter that is requires to properly reconstruct
        if self.multiheads:
            z0_extra = (
                torch.stack(
                    [self.extra_mlps[i].forward(z0_lin) for i in range(self.n_groups)]
                )
                .permute(1, 0, 2)
                .reshape((z0_lin.shape[0], -1))
            )
            z1_extra = (
                torch.stack(
                    [self.extra_mlps[i].forward(z1_lin) for i in range(self.n_groups)]
                )
                .permute(1, 0, 2)
                .reshape((z1_lin.shape[0], -1))
            )
        else:
            z0_extra = self.extra_mlp(z0_lin)
            z1_extra = self.extra_mlp(z1_lin)

        # [B, D] --> [B, G, D_P], D_P = D/G
        z0 = self.split(z0_lin)
        z1 = self.split(z1_lin)
    ...
    def forward_single(self, imgs, discriminator=False, group_id=0):
        ...
        if discriminator:
            ...
            fake_data, max_vals = self.get_fake_data(z_flat, preds)
    ...
    def get_fake_data(self, z, preds):
        max_vals = torch.max(preds, dim=0)[0]
        return (
            torch.cat(
                [
    ...
    def encode(self, x):
        ...
        z_lin = self._vt_linear(z_vt.reshape(z_vt.shape[0], self.vt_flat))
        return z_lin, z_vt
