...
class iCSN(nn.Module):
    def __init__(
        ...
        self._discriminator = nn.Sequential(
            nn.Linear(self.lin_enc_size + self.n_attrs, 512),
            nn.Mish(),
            nn.Linear(512, 512),
            nn.Mish(),
            nn.Linear(512, 1),
    ...
    def forward_single(self, imgs, discriminator=False):
        ...
        # x: [B, 3, 64, 64] --> [B, F, W, H] --> [B, D]
        enc_z = self.encode(imgs)

        z_extra = self.extra_mlp(enc_z)
        ...
        if discriminator:
            ...
            z_flat = torch.flatten(enc_z, start_dim=1)
            # idx = torch.randperm(preds.nelement())
            # shuffled_preds = preds.view(-1)[idx].view(preds.size())
            # shuffled_preds = torch.roll(preds,1, dims=1).clone().detach()
    ...
    def softmax_dot_product(self, enc, protos):
        ...
        norm_factor = torch.sum(
            torch.cat(
                [
                    torch.exp(
                        torch.sum(enc * protos[i], dim=1) / np.sqrt(self.proto_dim)
                    ).unsqueeze(dim=1)
