...
def train(model, data_loader, log_samples, optimizer, scheduler, writer, cur_epoch, config):
    ...
    for e in range(cur_epoch, config['epochs']):
        ...
        start = time.time()
        loss_dict = dict(
            {'loss': 0, "proto_recon_loss": 0, "inter_recon_loss": 0, "discriminator_loss": 0, "slot_loss": 0})
        param_dict = dict(
        ...
        for i, batch in enumerate(data_loader):
            ...
            if len(proto_recons)>1:
                loss = config['lambda_recon_proto'] * recon_loss_z0_proto + config['lambda_recon_proto'] * inter_recon_loss + config['gamma_discriminator'] * disc_loss + config['gamma_slots'] * slot_loss
            else:
            ...
            param_dict['disc_real_pred'] += len(disc_real_pred)
            param_dict['disc_fake_pred'] += len(disc_fake_pred)
