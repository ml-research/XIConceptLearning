...
def plot_examples(log_samples, rec_prototypes, writer, e, config):
    # apply encoding and decoding over a small subset of the training set
    imgs = log_samples
    examples_to_show = len(log_samples)

    rec_prototypes = rec_prototypes.detach().cpu()
    ...
    for i in range(examples_to_show):
        ...
        a[1][i].imshow(rec_prototypes[i].reshape(config['img_shape']).permute(1, 2, 0).squeeze(),
                       cmap='gray',
                       interpolation='none')
        a[1][i].axis('off')
