...
def train(model, data_loader, log_samples, optimizer, scheduler, writer, cur_epoch, config):
    ...
    for e in range(cur_epoch, config['epochs']):
        ...
        for i, batch in enumerate(data_loader):
            ...
            labels0_one_hot = torch.cat((
                utils.gen_one_hot_from_ids(imgs0.shape[0],
                                                 config['prototype_vectors'][0],
                                                 labels0_ids[:, 0].long(),
                                                 device=config['device']),
                utils.gen_one_hot_from_ids(imgs0.shape[0],
                                                  config['prototype_vectors'][1],
                                                  labels0_ids[:, 1].long(),
                                                  device=config['device']),
                utils.gen_one_hot_from_ids(imgs0.shape[0],
                                                  config['prototype_vectors'][2],
                                                  labels0_ids[:, 2].long(),
                                                  device=config['device'])
            ), dim=1)
            labels1_one_hot = torch.cat((
                utils.gen_one_hot_from_ids(imgs1.shape[0],
                                                 config['prototype_vectors'][0],
                                                 labels1_ids[:, 0].long(),
                                                 device=config['device']),
                utils.gen_one_hot_from_ids(imgs1.shape[0],
            ...
            rr_loss0 = F.mse_loss(preds[0][:, :18], labels0_one_hot)
            rr_loss1 = F.mse_loss(preds[1][:, :18], labels1_one_hot)
            ave_rr_loss = (rr_loss0 + rr_loss1)/2
...
def load_pretrained_state_dict(_model, state_dict):
    ...
    for name, param in state_dict.items():
        if "_encoder" in name or "split_mlp" in name:
...
def main(config):
    ...
    if config['ckpt_fp'] is not None:
        ...
        for key in ckpt['model_misc']['prototypes']:
            _model.proto_dict[key] = ckpt['model_misc']['prototypes'][key]
        _model.softmax_temp = ckpt['model_misc']['softmax_temp']
        print(_model.softmax_temp)
